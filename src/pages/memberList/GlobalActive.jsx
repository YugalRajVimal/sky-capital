...
const GlobalTeamTable = () => {
  ...
  useEffect(() => {
    const fetchDirectTeam = async () => {
      const data = await getTeamDetails(2);
      setDirectTeam(data.allActiveUsers);
    };
    fetchDirectTeam();
  }, []);
  ...
  const currentPageData = filteredTeam.slice(startIndex, endIndex);

  useEffect(() => {
    setCurrentPage(1); // Reset to first page on entries or search change
  }, [entries, search]);
  ...
  return (
    <div className="grow-1 w-full h-full  p-3 sm:p-5 rounded-2xl bg-[#030626] shadow-lg overflow-y-auto">
